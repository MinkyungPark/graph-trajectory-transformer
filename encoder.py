...
class GraphormerEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.n_vt = config.num_virtual_tokens # num virtual tokens
    ...
    def forward(self, x, perturb=None):
        bs, n, f = x.size()
        attn_bias, rel_pos = self.graph_attr.attn_bias.to(x.device), self.graph_attr.rel_pos.to(x.device)
        in_degree, out_degree = self.graph_attr.in_degree.to(x.device), self.graph_attr.out_degree.to(x.device)
        edge_input, attn_edge_type = self.graph_attr.edge_input.to(x.device), self.graph_attr.attn_edge_type.to(x.device)

        graph_attn_bias = attn_bias.clone()
        # (nH, N+1, N+1)
        graph_attn_bias = graph_attn_bias.unsqueeze(0).repeat(self.n_head, 1, 1)
        
        # rel_pos (N, N, nH) -> (nH, N, N)
        rel_pos_bias = self.rel_pos_encoder(rel_pos).permute(2, 0, 1)
        graph_attn_bias[:, :-self.n_vt, :-self.n_vt] = (
            graph_attn_bias[:, :-self.n_vt, :-self.n_vt] + rel_pos_bias
        )

        # reset rel pos here
        t = self.graph_token_virtual_distance.weight.view(self.n_head, self.n_vt).unsqueeze(-2)
        graph_attn_bias[:, :-self.n_vt, :-self.n_vt] = (
            graph_attn_bias[:, :-self.n_vt, :-self.n_vt] + t
        )

        # (N, N, 1) -> (N, N, nh) -> (nh, N, M)
        edge_input = self.edge_encoder(attn_edge_type).mean(-2).permute(2, 0, 1)
        
        graph_attn_bias[:, :-self.n_vt, :-self.n_vt] = (
            graph_attn_bias[:, :-self.n_vt, :-self.n_vt] + edge_input
        )
        graph_attn_bias = graph_attn_bias + attn_bias.unsqueeze(0) # reset
        ...
        in_degree = in_degree.unsqueeze(0).repeat(bs, 1)
        out_degree = out_degree.unsqueeze(0).repeat(bs, 1)
